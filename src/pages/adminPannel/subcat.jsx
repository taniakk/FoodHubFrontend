...
function SubCategory() {
    ...
    async function getsubCat() {
        try {
            const res = await axios.get(`${import.meta.env.VITE_SERVER}/cat/getcat`)
    ...
    const handleSubmit = async (e) => {
        ...
        // console.log("SubCategory:", selectedSubCat)

        // You can call API here
          try { 
                    const res = await axios.post(`http://localhost:3000/subCat/${selectedCat}`, {subcategory:subcategory})
    ...
    return (
        <div className=" h-screen w-full flex items-center justify-evenly">
            <form className="shadow-2xl h-[50%] w-[40%] rounded-4xl flex flex-col text-gray-800 items-center ml-20" onSubmit={handleSubmit}>
                ...
                <select className="my-2 p-2 rounded-2xl" name="subcategory" id="subcategory" onChange={(e) => setSelectedCat(e.target.value)}>
                ...
                <select className="my-2 p-2 rounded-2xl" name="subcategory" id="subcategory" onChange={(e) => setsubcategory(e.target.value)}>
                    <option value="">select option</option>
